...
carries parts that only make sense there: a CD4052 mux for selecting between
...
| `ASW_S0` | GPIO10 | GPIO1 |
| `ASW_S1` | GPIO20 | GPIO2 |
| `RESET_ALL` (originally supplied as `DWM_0_RST`) | GPIO21 | GPIO7 |
...
Four of the pins revision 2 left free drive a DRV8833, one H-bridge per winding:
...
Open `http://<probe>/stepper` for a trackpad: drag left or right to jog, let go
to stop. The page is served from flash by the same HTTP server as the API, in
one file with no external references — the station is reached over its own
...
| --- | --- | --- |
| `/api/v1/stepper` | GET | — |
| `/api/v1/stepper/jog` | POST | `{"steps_per_s":-250}` |
| `/api/v1/stepper/move` | POST | `{"steps":200,"steps_per_s":400}` |
| `/api/v1/stepper/stop` | POST | — |
| `/api/v1/stepper/release` | POST | — |

Two things it does on its own, both because a DRV8833 has no current chopping
and a stepper standing still with its windings energised is a resistor:
...
- **A jog expires.** `jog` is honoured for 400 ms and must be refreshed. If the
  ...
  heard was "keep going" — and a stepper that keeps going because nobody said
  stop is how a station drives itself into its end stop.
...
the STM32 and both DWM modules. `reset-cycle` alternates between those states
...
on STM32/DWM channels whose released SWDIO is high. It always restores the
...
board; through the CD4052 the mux settling time, not the engine, sets the
ceiling.
...
input so the IR board's 10 kΩ pull-up performs the release.
...
This firmware grew on an IR control board that puts a CD4052 analog
...
curl -X POST http://PROBE_IP/api/v1/mux/gps        # ...or the others
curl -X POST http://PROBE_IP/api/v1/dwm0/reset     # compatibility path: RESET_ALL
